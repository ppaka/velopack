...
public interface IVelopackServiceClient
{
...
public class VelopackServiceClient(HttpClient HttpClient, IConsole Console) : IVelopackServiceClient
{
    ...
    public async Task<bool> LoginAsync(VelopackLoginOptions? options = null)
    {
        ...
        AuthConfiguration authConfiguration = await GetAuthConfigurationAsync(options);
    ...
    public async Task UploadReleaseAssetAsync(UploadOptions options)
    {
        ...
        Uri endpoint = GetEndpoint("api/v1/upload", options);
    ...
    private void AssertAuthenticated()
    {
        if (!HasAuthentication) {
            throw new InvalidOperationException($"{nameof(VelopackServiceClient)} has not been authenticated, call {nameof(LoginAsync)} first.");
        }
    }
